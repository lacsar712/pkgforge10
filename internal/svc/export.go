...
import (
	...
	"example.com/pkgforge/internal/util/filedump"
	"example.com/pkgforge/internal/util/jsonx"
)
...
func (c *Catalog) ExportJSON(ctx context.Context) ([]byte, error) {
	...
	exp := model.ExportFile{Name: "Pkgforge APT", Age: len(recs)}
	...
	snap := Snapshot{
		...
		Records: recs,
		Export:  decoded,
		Meta:    map[string]string{"app": "pkgforge"},
	}
